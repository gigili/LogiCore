...
public class ServerItem extends Item {
    public ServerItem(Item.Properties properties) {
        super(properties
                .stacksTo(1)
    ...
    public static ShapedRecipeBuilder getRecipe(HolderGetter<Item> items) {
        return ShapedRecipeBuilder.shaped(items, RecipeCategory.REDSTONE, ServerModule.SERVER.get())
                .pattern("IRI")
                .pattern("QPQ")
                .pattern("GRG")
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext context, @NotNull TooltipDisplay display, @NotNull Consumer<Component> tooltipComponents, @NotNull TooltipFlag tooltipFlag) {
        ...
        if (cpuCount.containsKey(ProcessorUnitTier.BASIC.name)) {
            tooltipComponents.accept(Component.translatable("cpu.tooltip.logicore.basic_tier_count", cpuCount.get(ProcessorUnitTier.BASIC.name)));
        }

        if (cpuCount.containsKey(ProcessorUnitTier.ADVANCED.name)) {
            tooltipComponents.accept(Component.translatable("cpu.tooltip.logicore.advance_tier_count", cpuCount.get(ProcessorUnitTier.ADVANCED.name)));
        }

        if (cpuCount.containsKey(ProcessorUnitTier.ULTIMATE.name)) {
            tooltipComponents.accept(Component.translatable("cpu.tooltip.logicore.ultimate_tier_count", cpuCount.get(ProcessorUnitTier.ULTIMATE.name)));
        }

        tooltipComponents.accept(Component.translatable("item.logicore.server.total_generation_tooltip", totalGenerationCount));
    }
}
